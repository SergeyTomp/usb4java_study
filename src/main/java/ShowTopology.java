...
{
    public static void main(String argv[]) throws UnsupportedEncodingException, UsbException {
     ...
     */
    public static void processUsingGetAttachedUsbDevices(UsbDevice usbDevice, String prefix) throws UnsupportedEncodingException, UsbException {
        UsbHub usbHub = null;

        /* If this is not a UsbHub, just display device and return. */
        if (!usbDevice.isUsbHub()) {
            System.out.println(prefix + "Device " + UsbUtil.toHexString(usbDevice.getUsbDeviceDescriptor().idProduct()) + " " + UsbUtil.toHexString(usbDevice.getUsbDeviceDescriptor().idVendor()) );
            return;
        } else {
